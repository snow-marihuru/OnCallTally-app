...
OnCallTally-app は、電話当番（オンコール）表から集計期間内の当番日数を「平日／土曜日／日祝日」区分別に人ごとへ自動集計し、Excel（.xlsx）で出力するWebアプリ。給与計算（担当日数のカウント）の手作業・ミスをなくすことが目的。将来的にはWeb UIを外してローカル運用へ切り替える構想があるため、集計ロジックとUIを分離した構成にしている。

Excelには手当支給額（平日600円／土曜900円／日祝日1200円の単価、`core.ALLOWANCE_RATES`）も出力するが、**これは実際の支給額を決定するためのものではない**。実際の支給額計算・確定は給与システム側で行っており、本アプリの手当額は現状手計算で行っている検算作業を、給与システムの計算結果とのクロスチェック（一致確認）用に自動化したもの。単価が変わった場合はこの定数を更新すること。
